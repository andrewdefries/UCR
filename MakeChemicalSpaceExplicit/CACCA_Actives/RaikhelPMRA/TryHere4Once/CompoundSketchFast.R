...
library(hwriter)
#####################
p=openPage(html_name)	#opens the page wrapper 
##
load("nnm.rda")		#gets our distance data
##
colnames(Fix)<-1:length(colnames(Fix))
##
#colors<=c('#000000','#FFFFFF')
##
#ImageURLs<-read.table()
#OtherURLs
#col.link=list(Sepal.Width=iris$Sepal.Width[1:7]))
##
hwrite(Fix, p, br=TRUE, col.link="http://spark.rstudio.com/adefries/AcetyleneActives/") #make vector to hold urls
##
closePage(p)
##################
